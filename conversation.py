...
# meditron starts here 

overall_system_msg = """You are a helpful medical assistant, and patients ask you questions about their personal healthcare data. Your responses should be concise and clear, providing helpful information based on the context
   given on the patient's health record."""

system_msg_task_1 = """You will be given a query and a resource from a patient's medical record.
 Your job is to determine if the resource would be relevant in providing an answer to the query based
   on matching medical conditions, treatments, or dates. Respond with 'True' if the resource is relevant to providing an answer to the patient's query,
     or 'False' if it is not."""

system_msg_task_2 = """You will be given a snippet from a patient's medical record in JSON format. your job is to
 provide a 1-2 sentence summary of the provided JSON in a way that would be helpful for a doctor to understand 
 the data point in the patient's medical record. You should avoid making assumptions about the patient's medical record
 and only provide an answer based on the summary. If the patient inquires about a more general medical question, you may 
 answer it to the best of your ability."""


system_msg_task_3 = """You will be given a query from a patient curious about their medical record and a list of short summaries
 of data points in a patient's medical record. Your job is to provide a concise and clear response to the query based on the
   information in the summaries. If the summaries do not contain enough details (such as specific treatments, diagnosis, or dates
     relevant to the query), you should inform the user that you were unable to find the answer based on their medical record. Avoid making assumptions about the patient's medical record and only provide an answer based on the summaries."""
...
register_conv_template(
   Conversation(
        name="task_1",
        system_message=system_msg_task_1
        roles=("User", "Assistant"),
        messages=(
            (
                "User",
                "Query: When was the last time I had a general examination? Resource: Encounter General examination of patient (procedure) 04-28-1970",
            ),
            (
                "Assistant",
                "True",
            ...
            (
                "User",
                "Query: When was the last time I had a general examination? Resource: Procedure Electrical cardioversion 02-09-2021",
            ),
            (
                "Assistant",
                "False",
            ...
            (
                "User",
                "Query: What Medication did I recently refill that prevents blood clots? Resource: MedicationRequest Warfarin Sodium 5 MG Oral Tablet 01-23-2024",
            ),
            (
                "Assistant",
                "True",
            ...
            (
                "User",
                "Query: What Medication did I recently refill that prevents migraines? Resource: MedicationRequest Digoxin 0.125 MG Oral Tablet 12-14-2010",
            ),
            (
                "Assistant",
                "False",
...
register_conv_template(
    Conversation(
        name="medical_resource_relevance",
        system_message=system_msg_task_2,
        roles=("User", "Assistant"),
        messages=(
            (
                "User",
                "Resource: Encounter General examination of patient (procedure) 04-28-1970",
            ),
            (
                "Assistant",
                "The patient had a general examination on 04-28-1970.",
            ),
            (
                "User",
                "Resource: MedicationRequest Warfarin Sodium 5 MG Oral Tablet 12-02-2024",
            ),
            (
                "Assistant",
                "The patient requested to refil Warfarin Sodium 5 MG Oral Tablet on 12-02-2024.",
            ),
            (
                "User",
                "Resource: Immunization Hep A, ped/adol, 2 dose 10-08-2017",
            ),
            (
                "Assistant",
                "The patient recieved a vaccine for Hep A, ped/adol, 2 doses on 10-08-2017.",
            ),
            (
                "User",
                "Resource: Immunization Hep A, ped/adol, 2 dose 10-08-2017",
...
# for below, using example with data, we don't need to recall data every time but we do wanna make sure that it can be used

register_conv_template(
    Conversation(
        name="two_shot_patient",
        system_message=system_msg_task_3,
        roles=("User", "Assistant"),
        messages=[
            # FILL IN HERE
            (
                "User",
                "When should I get my flu shot again?",
            ),
            (
                "Assistant",
                """Given your last flu shot was recorded on February 9th, 2024, it is recommended to get vaccinated annually. Considering the date of your last vaccination, you should plan to receive your next flu shot in the upcoming flu season, starting in October.""",
            ),
            (
                "User",
                "I had blood tests and an ultrasound for abdominal pain but nothing was found. Now I'm worried I might have Hodgkin's disease because I've been feeling slightly itchy.",
            ),
            (
                "Assistant",
                """It's natural to be concerned about your health, especially with unexplained symptoms. However, abdominal pain and itchiness can stem from many benign causes. Hodgkin's disease is characterized by specific symptoms, such as painless swelling of lymph nodes. It's important to continue consultations with your healthcare provider for a thorough evaluation and to discuss your concerns about Hodgkin's disease. They may recommend additional tests or specialist referrals if necessary. Remember, direct communication with your medical provider is key to understanding and addressing your health concerns.""",
            )
        ],
        offset=2,
        sep_style=SeparatorStyle.ADD_COLON_SINGLE,
